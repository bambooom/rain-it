...
class RainDrop {
     ...
     */
    constructor(emoji, maxHeight, maxWidth, maxVelocity = 5, mode = RAINIT_MODE.RAIN) {
        this.text = emoji;
        this.x = Math.random() * maxWidth;
        this.y = Math.random() * maxHeight;
        this.vx = mode === RAINIT_MODE.RAIN ? 0 : Math.random() * maxVelocity;
        this.vy = mode === RAINIT_MODE.RAIN ? AG : Math.random() * maxVelocity;
...
class RainIt {
    constructor() {
        ...
        this.EmojiNum = RAINIT_EMOJI_CODE.length;
    }
     ...
     */
    getOrCreateCanvas() {
        if (document.getElementById('rain-it')) {
            return document.getElementById('rain-it');
        }
        const canvas = document.createElement("canvas");
        canvas.id = 'rain-it';
        canvas.height = window.innerHeight;
        canvas.width = window.innerWidth;
        canvas.style.cssText = "position: fixed; background: transparent; overflow: hidden; left: 0; right: 0; top: 0; bottom:0; visibility: hidden; margin: 0; padding: 0;";
    ...
    /** 
     * @param {string} mode - wiethe it's in single or mixed emoji
     * @return {RainDrop[]} - array of raindrops, num is maxDrops
     */
    generateEmojiDrops(mode = RAINIT_EMOJI_SINGLE_MIXED.SINGLE) {
        const rainDrops = [];
        switch (mode) {
            case RAINIT_EMOJI_SINGLE_MIXED.SINGLE:
                {
                    const emoji = this.getEmojiFromCodePoint(RAINIT_EMOJI_CODE[Math.floor(Math.random() * this.EmojiNum)]);
                    for (let i = 0; i < RAINIT_MAX_DROPS; i++) {
                        rainDrops.push(new RainDrop(emoji, this.canvas.height, this.canvas.width));
                    }
                    break;
                }
            case RAINIT_EMOJI_SINGLE_MIXED.MIXED:
                {
                    for (let i = 0; i < RAINIT_MAX_DROPS; i++) {
                        const emoji = this.getEmojiFromCodePoint(RAINIT_EMOJI_CODE[Math.floor(Math.random() * this.EmojiNum)]);
                        rainDrops.push(new RainDrop(emoji, this.canvas.height, this.canvas.width));
    ...
    start(rainDrops) {
        ...
        window.RAINIT_LOOPID = setInterval(this.drawRain.bind(this), 50, rainDrops);
    }

    stop() {
        clearInterval(window.RAINIT_LOOPID);
        window.RAINIT_LOOPID = null;
        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
...
(function setup() {
    const rainMaker = new RainIt();
    rainMaker.resize();
    window.addEventListener('resize', () => rainMaker.resize());

    chrome.runtime.onMessage.addListener(function(message, sender, callback) {
        if (message === 'TOGGLE_RAIN') {
            const rainDrops = rainMaker.generateEmojiDrops();
            rainMaker.start(rainDrops);
        } else {
            rainMaker.stop();
        }
    });
